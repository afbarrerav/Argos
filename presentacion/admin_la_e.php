...
<table border="0" width="100%" align="center" style="background: #FFF; border: 1px solid #CCC;">
	...
	<tr>
		<td  colspan ="2">
			<img src="imagenes/iconos/save_a.png" alt="Guardar" title="Guardar"
				...
				onclick="AjaxConsulta( '../logica/admin_ic.php', {TABLA:'<?php echo $tabla?>', <?php echo $parametros?> ACCION:'editar_ic'}, 'administarICDiv' );"/>
	...
	<tr>
		...
		<td class="texto_tabla" width="50%">
			<?php 
			if(($atributos[$l]!="est_codigo")&&($atributos[$l]!="dep_codigo")&&($atributos[$l]!="pai_codigo"))
			{?>
			...
			else 
			{
				...
				else 
				{
					...
					{
						?>
							<script>AjaxConsulta( '../logica/proc_select_tabla.php', {TABLA_CONSULTAR:'departamentos', TABLA_ACTUALIZAR:'0', VALOR_REGISTRO:'<?php echo $registros[$atributos[$l]][$m]?>', CODIGO_REGISTRO:'<?php echo $registros['codigo'][$m]?>', NOMBRE_CAMPO:'nombre', DIV:'departamentoDiv', NOMBRE_SELECT:'<?php echo $atributos[$l];?>', ACCION:'consultar_campo'}, 'departamentoDiv' );</script><div id="departamentoDiv"></div>
						<?php 
					}	
					else 
					{
						...
						{
							?>
								<script>AjaxConsulta( '../logica/proc_select_tabla.php', {TABLA_CONSULTAR:'paises', TABLA_ACTUALIZAR:'0', VALOR_REGISTRO:'<?php echo $registros[$atributos[$l]][$m]?>', CODIGO_REGISTRO:'<?php echo $registros['codigo'][$m]?>', NOMBRE_CAMPO:'nombre', DIV:'paisDiv', NOMBRE_SELECT:'<?php echo $atributos[$l];?>', ACCION:'consultar_campo'}, 'paisDiv' );</script><div id="paisDiv"></div>
							<?php 
						}
					}
				}
