...
AND admin_jva_usuarios.codigo =$codigo";
	...
	function consulta_Rutas($vendedor, $fecha_venta, $usuario, $pwd)
	{
   	...
   	function ingreso_Clientes_Offline($ti_codigo,$nroidentificacion,$razon_social,$tn_codigo,$nombre1_contacto,
   	$nombre2_contacto,$apellido1_contacto,$apellido2_contacto,$telefono1,$ext1,$telefono2,$ext2,$celular1,$celular2,
   	$email,$barrio,$direccion,$comentario,$ciu_codigo,$est_codigo, $usuario, $pwd)
	{
                global $DATABASE_NAME;
                global $dsn ;
                $username=$usuario;
		...
		VALUES ('$ti_codigo','$nroidentificacio','$referencia','$razon_social','$tn_codigo','$nombre1_contacto',
		'$nombre2_contacto','$apellido1_contacto','$apellido2_contacto','$telefono1','$ext1','$telefono2','$ext2','$celular1','$celular2',
		'$email','$barrio','$direccion','$comentario','$calificacion','$ciu_codigo','$est_codigo')";
		/*
		 * SE PREPARA EL QUERY
		 * */
		$result = $db_link->prepare($query);
	...
	function ingreso_Solicitud_Offline($codigo_barras,$referencia,$fecha_solicitud,$cli_codigo,$aju_codigo,$ttp_codigo,$tp_codigo,$tnc_codigo,$valor_producto,$valor_comision_servicio,
	$valor_impuesto,$valor_total,$fecha_entrega,$est_codigo, $usuario, $pwd)
	{
                global $DATABASE_NAME;
                global $dsn ;
                $username=$usuario;
		...
		$query = "INSERT INTO trans_ventas(codigo_barras, referencia, fecha_solicitud, cli_codigo, aju_codigo, ttp_codigo, tp_codigo, 
tnc_codigo, valor_producto, valor_comision_servicio, valor_impuesto, valor_total, fecha_entrega, est_codigo) 
VALUES ( $codigo_barras,$referencia,$fecha_solicitud,$cli_codigo,$aju_codigo,$ttp_codigo,$tp_codigo,$tnc_codigo,$valor_producto,$valor_comision_servicio,
$valor_impuesto,$valor_total,$fecha_entrega,$est_codigo )";
	...
	function ingreso_Transaccion_Offline($tv_codigo,$aju_codigo,$cuotas_totales,$cuota_nro,$fecha_pago,
	$valor_pago,$fecha_recaudo,$valor_recaudo,$fecha_trans,$longitud,$latitud,$est_codigo, $usuario, $pwd)
	{
                global $DATABASE_NAME;
                global $dsn ;
                $username=$usuario;
			...
			$query = "UPDATE trans_detalle_recaudo_ventas_jva SET tv_codigo=$tv_codigo,
			aju_codigo=$aju_codigo,cuotas_totales=$cuotas_totales,cuota_nro=$cuota_nro,fecha_pago=$fecha_pago,
			valor_pago=$valor_pago,fecha_recaudo=$fecha_recaudo,valor_recaudo=$valor_recaudo,fecha_trans=$fecha_trans,
			longitud=$longitud,latitud=$latitud,est_codigo=$est_codigo
	...
	function consulta_Trans_Ventas($aju_codigo,$usuario,$pwd)
	{
		...
		$query = "SELECT  codigo, codigo_barras,  referencia ,  fecha_solicitud ,  cli_codigo ,  aju_codigo ,  ttp_codigo ,  
		tp_codigo ,  tnc_codigo , valor_producto ,  valor_comision_servicio , valor_impuesto ,  valor_total ,  
		fecha_entrega ,  longitud ,  latitud ,  est_codigo 
	...
	function trans_detalle_recaudo_ventas_jva($aju_codigo,$usuario,$pwd)
	{
		...
		$query = "SELECT codigo, tv_codigo, aju_codigo, cuotas_totales, cuota_nro, fecha_pago, valor_pago, fecha_recaudo, valor_recaudo, 
		fecha_trans, longitud, latitud, est_codigo FROM trans_detalle_recaudo_ventas_jva WHERE aju_codigo = $aju_codigo limit 0,3511";
