...
try
{
		...
		if($error_jva == "00000")
		{
			...
			$db_link->commit();		
		 	...
		 	{	
				 ...
				 * */
				$query_rutas = "SELECT count(codigo) cantidad_rutas
				...
				{					
					$db_link->commit();					
					$row_rutas = $result_rutas->fetch(PDO::FETCH_ASSOC);
	 				$cantidadRutas = $row_rutas['cantidad_rutas'];
	 				//echo "Cantidad de Rutas $cantidadRutas: ".date('Y-m-d H:i:s')."<br>";
					/*
					 * OBTIENE LOS PRODUCTOS DE LA RUTA
					 * */
	 				$query_ventas = "SELECT count(tv_codigo) cantidad_productos 
	 								FROM trans_rutas_detalles 
	 								WHERE trj_codigo in (select codigo from trans_rutas_jva where jva_codigo =  $jva_codigo) 
	 								AND est_codigo = 1";
					...
					if($error_ventas == "00000")
					{
						...
						$cantidadProdutos = $row_ventas['cantidad_productos'];
						...
						$query_recaudo = 	"SELECT sum(valor_pago) valor_pago
											FROM (SELECT distinct tv_codigo, valor_pago
												...
												WHERE tv_codigo in (SELECT tv_codigo FROM trans_rutas_detalles WHERE trj_codigo in (select codigo from trans_rutas_jva where jva_codigo =  $jva_codigo) and est_codigo = 1)
						...
						$query_recaudado = "select count(tv_codigo) nro_clientes_recaudados, sum(valor_recaudo) valor_recaudo
											from (SELECT tv_codigo, fecha_recaudo, sum(valor_recaudo) valor_recaudo
												...
												WHERE tv_codigo in (select tv_codigo from trans_rutas_detalles where trj_codigo in (select codigo from trans_rutas_jva where jva_codigo =  $jva_codigo))
	...
	if($accion == "consultar_ruta_recaudos")
	{
		...
		$query = "select codigo trj_codigo, nombre ruta, aju_codigo, nombre_vendedor, count(cli_codigo) clientes_recaudados, sum(valor_pago) valor_recaudado
					from (SELECT trj.codigo, trj.nombre, trj.jva_codigo, trj.aju_codigo, tv.valor_producto AS valor_total, au.codigo AS codigo_vendedor, au.nombres AS nombre_vendedor, au.apellidos AS apellido_vendedor, tv.codigo AS tv_codigo, ac.codigo cli_codigo, CONCAT(ac.nombre1_contacto, ' ', apellido1_contacto) as razon_social, tdrvj.fecha_recaudo, SUM(tdrvj.valor_recaudo) as valor_pago
					         ...
					         WHERE trj.jva_codigo IN (select jva_codigo from admin_jva_usuarios where rol_codigo in (2, 3) and usu_codigo =  $usu_codigo)
	...
	if($accion == "consultar_recaudos_socio")
	{
		...
		$query = "select ar.codigo, ar.nombre, ar.descripcion, ar.jva_codigo, ar.aju_codigo, fun_obtener_aju_nombre(ar.aju_codigo) aju_nombre, ar.pbj_codigo, fun_nro_clientes_ruta(ar.codigo) nro_clientes, ar.saldo, ar.est_codigo
					...
					and ar.jva_codigo IN (select jva_codigo from admin_jva_usuarios where rol_codigo in (2, 3) and usu_codigo =  $usu_codigo)
